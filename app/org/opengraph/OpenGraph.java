...
public class OpenGraph {
	...
	private boolean isImported; // determine if the object is a new incarnation
								// or representation of a web page
	private boolean hasChanged; // track if object has been changed

	public final static String[] REQUIRED_META = new String[] { "title",
			"type", "image", "url" };
	...
	static {
		...
		BASE_TYPES.put("business", new String[] { "bar", "company", "cafe",
				"hotel", "restaurant" });
		BASE_TYPES.put("group", new String[] { "cause", "sports_league",
				"sports_team" });
		BASE_TYPES.put("organization", new String[] { "band", "government",
				"non_profit", "school", "university" });
		BASE_TYPES.put("person", new String[] { "actor", "athlete", "author",
				"director", "musician", "politician", "profile",
				"public_figure" });
		BASE_TYPES.put("place", new String[] { "city", "country", "landmark",
				"state_province" });
		BASE_TYPES.put("product", new String[] { "album", "book", "drink",
				"food", "game", "movie", "product", "song", "tv_show" });
		BASE_TYPES
				.put("website", new String[] { "blog", "website", "article" });
	}
	 ...
	 *
	 * @param url
	 *            The address to the web page to fetch Open Graph data
	 * @param ignoreSpecErrors
	 *            Set this option to true if you don't wish to have an exception
	 *            throw if the page does not conform to the basic 4 attributes
	 * @throws java.io.IOException
	 *             If a network error occurs, the HTML parser will throw an IO
	 *             Exception
	 * @throws java.lang.Exception
	 *             A generic exception is throw if the specific page fails to
	 *             conform to the basic Open Graph standard as define by the
	...
	public OpenGraph(String url, boolean ignoreSpecErrors)
		...
		// download the (X)HTML content, but only up to the closing head tag. We
		// do not want to waste resources parsing irrelevant content
		URL pageURL = new URL(url);
		URLConnection siteConnection = pageURL.openConnection();
		siteConnection
				.setRequestProperty(
						"User-Agent",
						"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/46.0.2490.80 Safari/537.36");// ヘッダを設定
		...
		if (headElement.hasAttribute("prefix")) {
			...
			Pattern pattern = Pattern
					.compile("(([A-Za-z0-9_]+):\\s+(http:\\/\\/ogp.me\\/ns(\\/\\w+)*#))\\s*");
		...
		if (!hasOGspec)
			pageNamespaces.add(new OpenGraphNamespace("og",
					"http:// ogp.me/ns#"));
		...
		for (TagNode metaElement : metaData) {
			for (OpenGraphNamespace namespace : pageNamespaces) {
				...
				if (target != null
						&& metaElement.getAttributeByName(target).startsWith(
								namespace.getPrefix() + ":")) {
		...
		if (!ignoreSpecErrors) {
			for (String req : REQUIRED_META) {
				if (!metaAttributes.containsKey(req))
					throw new Exception(
							"Does not conform to Open Graph protocol");
			}
		}
		...
		if (currentType != null) {
			for (OpenGraphNamespace ns : pageNamespaces) {
				if (currentType.startsWith(ns.getPrefix() + ":")) {
					currentType = currentType.replaceFirst(
							ns.getPrefix() + ":", "");
					break; // done here
				}
			}
	...
	public String getContent(String property) {
		if (metaAttributes.containsKey(property)
				&& metaAttributes.get(property).size() > 0)
	...
	public MetaElement[] getProperties() {
		...
		return (MetaElement[]) allElements.toArray(new MetaElement[allElements
				.size()]);
	}
	...
	public MetaElement[] getProperties(String property) {
		if (metaAttributes.containsKey(property)) {
			...
			return (MetaElement[]) target
					.toArray(new MetaElement[target.size()]);
		} else
			return null;
	}
	 ...
	 *
	 * @param namespace
	 *            The OpenGraph namespace the content belongs to
	 * @param property
	 *            The og:XXXX where XXXX is the property you wish to set
	 * @param content
	 *            The value or contents of the property to be set
	 */
	public void setProperty(OpenGraphNamespace namespace, String property,
			String content) {
	 ...
	 *
	 * @param property
	 *            The og:XXXX where XXXX is the property you wish to remove
	 */
